...
void calcGreyscale(const Mat& source, int window_width, int window_height, int* image, float* greyscale)
{
	//most of the stuff is global pointers so it'll be ok
...
void calcGradient(int window_width, int window_height, float* greyscale, float* vals)
{
	//calc gradient (val) for each pixel
	//y is row (height), x is column (width)
	for(int y = 0; y < window_height; y++)
...
bool seamcarve_v_cpu(int numSeams, const Mat& source, Mat& dest)
{
	// printf("start v cpu: source is %d rows by %d cols\n",source.rows,source.cols);
	unsigned int window_width = source.cols;
	unsigned int window_height = source.rows;
	unsigned long window_size = source.rows * source.cols;
	...
	calcGreyscale(source, window_width, window_height, image, greyscale);
	calcGradient(window_width, window_height, greyscale, vals);
