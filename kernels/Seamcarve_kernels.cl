...
{
	float min_val = costs[GPU_POSITION(0,0,window_width)];
	seam[0] = 0;
...
__kernel void vcolumnCalcCostsRev(__global float* costs, __global char* dirs, __global float* vals, unsigned int window_width, unsigned int window_height, unsigned int count)
{ 
	unsigned int x = get_global_id(0);
	if(x >= window_width - count)
		return;
...
__kernel void vcalcSeamToRemoveRev(__global float* costs, __global char *dirs, __global int* seam, unsigned int window_width, unsigned int window_height, unsigned int count, __global float* vmin)
{
	float min_val = costs[GPU_POSITION(0,0,window_width)];
	seam[0] = 0;
	for(unsigned int x = 1; x < window_width - count; x++)
	{
		if(costs[GPU_POSITION(x,0,window_width)] > min_val)
		{
			min_val = costs[GPU_POSITION(x,0,window_width)];
...
__kernel void hcalcSeamToRemove(__global float* costs, __global char *dirs, __global int* seam, unsigned int window_width, unsigned int window_height, unsigned int count, __global float* hmin)
{
	float min_val = costs[GPU_POSITION(0,0,window_width)];
	seam[0] = 0;
	for(unsigned int y = 1; y < window_height - count; y++)
	{
